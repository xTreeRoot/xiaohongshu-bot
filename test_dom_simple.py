...
def test_dom_simple():
    ...
    if retrieved_element:
        ...
        print(f"页面URL: {retrieved_element.page_url}")
    else:
        print("未获取到元素")
    ...
    print(f"从数据库获取的DOM元素数量: {len(selectors)}")
    ...
    if element:
        print(f"从数据库获取: {element.element_id}")
    
    # 使用DOM管理器获取（应该从缓存或数据库获取）
    cached_element = dom_manager.get_element("#login-btn")
    if cached_element:
        print(f"通过DOM管理器获取: {cached_element.element_id}")
