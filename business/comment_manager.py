...
class CommentManager:
                ...
                try:
                    ...
                    if 'data' in data and 'comments' in data['data']:
                        ...
                        if first_comment and 'pictures' in first_comment:
                            print(
                                f"\n[DEBUG] 第一条评论的pictures字段结构: {json.dumps(first_comment.get('pictures', []), indent=2, ensure_ascii=False)}")
                except:
                    pass
    ...
    def _scroll_page(self, scroll_count=3, scroll_pause=2, note_id=None):
        ...
        try:
            ...
            try:
                ...
                for container in possible_containers:
                    ...
                    if info['hasScroll'] and info['overflowY'] in ['auto', 'scroll']:
                        ...
                        print(f"✓ 找到可滚动容器: {info['className'][:50]}")
                        print(f"  容器高度: {info['clientHeight']}px, 可滚动高度: {info['scrollHeight']}px")
                        break
            except Exception as e:
                print(f"查找可滚动容器失败: {e}")
            ...
            for i in range(scroll_count):
                ...
                print(f"✓ 第 {i + 1}/{scroll_count} 次滚动 (距离: {scroll_distance}px, 位置: {scroll_top_after}px)")
                    ...
                    try:
                        ...
                        if message['method'] == 'Network.responseReceived':
                            ...
                            if 'api/sns/web/v2/comment/page' in response_url:
                                ...
                                print(f"    检测到评论接口: {response_url[:80]}...")
                                ...
                                try:
                                    ...
                                    if 'body' in response_body:
                                        ...
                                        print(f"    ✓ 成功获取响应体 (ID: {request_id[:8]}...)")

                                        # 提取评论
                                        comments = self._extract_comments_from_response(response_body['body'])
                                        all_comments.extend(comments)
                                        print(f"    本次获取 {len(comments)} 条评论，累计 {len(all_comments)} 条")
                ...
                if new_comment_requests > 0:
                    print(f"  ✓ 本次滚动检测到 {new_comment_requests} 个评论接口请求")
                else:
                    if scroll_distance == 0:
                        print(f"    滚动距离为0且无新接口，已到底部")
                        print(f"  ✓ 提前结束滚动")
                        break
                    else:
                        print(f"  ⚠ 未检测到新的评论接口请求（可能正在加载中）")
    ...
    def fetch_comments(self, note_id=None, enable_scroll=False, scroll_count=3):
        ...
        try:
                    ...
                    if message['method'] == 'Network.responseReceived':
                        ...
                        if 'api/sns/web/v2/comment/page' in response_url:
                            ...
                            try:
                                ...
                                if 'body' in response_body:
                                    processed_request_ids.add(request_id)
                                    print(f"  ✓ 获取初始评论接口响应 (ID: {request_id[:8]}...)")

                                    # 提取评论
                                    comments = self._extract_comments_from_response(response_body['body'])
                                    all_comments.extend(comments)
                                    initial_comment_count += len(comments)
                                    print(f"  ✓ 获取 {len(comments)} 条初始评论")
                            except Exception as e:
                                error_msg = str(e)
                                if 'No resource with given identifier found' not in error_msg:
                                    print(f"  ✗ 获取失败: {error_msg}")
            ...
            print(f"✓ 初始加载完成，获取到 {initial_comment_count} 条评论\n")
            ...
            print(f"\n统计信息:")
            print(f"  - 初始加载获取 {initial_comment_count} 条评论")
            print(f"  - 滚动加载获取 {len(all_comments) - initial_comment_count} 条评论")
            print(f"  - 总共获取到 {len(all_comments)} 条评论\n")
    ...
    def print_comments(self, comments: List):
        ...
        print(f"\n{'=' * 80}")
        print(f"✓ 总共获取到 {len(comments)} 条评论")
        print(f"{'=' * 80}\n")

        for comment in comments:
            print(f"\n[评论]")
            print(f"  用户: {comment.user_info.nickname}")
            print(f"  内容: {comment.content}")
            print(f"  点赞数: {comment.like_count}")
            print(f"  IP归属地: {comment.ip_location}")
            print(f"  回复数: {comment.sub_comment_count}")
            print(f"  评论ID: {comment.comment_id}")

            # 打印语音评论
            if comment.audio_info:
                print(f"  [语音评论] {comment.audio_info.tag_text}")
                print(f"  识别文本: {comment.audio_info.asr_text}")

            # 打印图片
            if comment.pictures:
                print(f"  [图片评论] {len(comment.pictures)} 张图片")
                for idx, pic_url in enumerate(comment.pictures, 1):
                    print(f"    图片{idx}: {pic_url}")
            ...
            if comment.sub_comments:
                ...
                for idx, sub in enumerate(comment.sub_comments, 1):
                    print(f"\n  [回复 {idx}]")
                    print(f"    用户: {sub.user_info.nickname}")
                    print(f"    内容: {sub.content}")
                    print(f"    点赞数: {sub.like_count}")
                    print(f"    IP归属地: {sub.ip_location}")
                    print(f"    评论ID: {sub.comment_id}")

                    # 子评论的图片
                    if sub.pictures:
                        print(f"    [图片回复] {len(sub.pictures)} 张图片")
                        for pic_idx, pic_url in enumerate(sub.pictures, 1):
                            print(f"      图片{pic_idx}: {pic_url}")
                    ...
                    if sub.target_comment:
                        ...
                        print(f"    回复给: @{target_name} (ID: {target_id})")
            print(f"\n{'-' * 80}")
    ...
    def reply_to_comment(self, comment_id: str, reply_text: str) -> bool:
        ...
        try:
            ...
            print(f"  查找评论元素: {comment_selector}")
            ...
            print(f"  查找回复按钮: {reply_button_selector}")
