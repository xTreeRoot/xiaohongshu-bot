...
def test_integration():
    ...
    try:
        ...
        logger.info(f"数据库映射器: {dom_manager.mapper is not None}")
        logger.info(f"缓存管理器: {dom_manager.cache_manager is not None}")
        
        # 测试初始元素插入
        logger.info("\n2. 测试初始元素插入...")
        from core.config import config
        selectors = config.xhs.selectors
        logger.info(f"配置中的选择器数量: {len(selectors) if selectors else 0}")
        
        if selectors:
            dom_manager.batch_insert_initial_elements(selectors, "https://creator.xiaohongshu.com")
            logger.info("初始元素批量插入完成")
        
        # 测试搜索功能
        logger.info("\n3. 测试搜索功能...")
        all_elements = dom_manager.mapper.find_all()
        logger.info(f"数据库中总元素数量: {len(all_elements)}")
        
        selector_elements = dom_manager.mapper.find_by_type("selector")
        logger.info(f"类型为'selector'的元素数量: {len(selector_elements)}")
        
        for element in selector_elements[:5]:  # 只显示前5个
            logger.info(f"  - {element.element_id}: {element.selector}")
        
        # 测试单个元素的获取、更新、删除
        logger.info("\n4. 测试单个元素操作...")
        if selector_elements:
            first_element = selector_elements[0]
            logger.info(f"获取第一个元素: {first_element.element_id}")
            
            # 测试从缓存获取
            cached_element = dom_manager.get_element(first_element.selector)
            if cached_element:
                logger.info(f"从缓存获取成功: {cached_element.element_id}")
            ...
            logger.info(f"更新操作结果: {update_success}")
...
def test_browser_manager_dom_integration():
    ...
    try:
        ...
        # 测试DOM管理器是否正确初始化
        logger.info(f"BrowserManager DOM管理器已初始化: {hasattr(bm, 'dom_manager')}")
        logger.info(f"DOM管理器类型: {type(bm.dom_manager).__name__}")
        ...
        logger.info(f"插入测试元素结果: {insert_result}")
        
        get_result = bm.dom_manager.get_element(".integration-test")
        logger.info(f"获取测试元素结果: {get_result is not None}")
        
        if get_result:
            logger.info(f"获取的元素ID: {get_result.element_id}")
        
        logger.info("BrowserManager DOM功能测试完成！")
        
    except Exception as e:
        logger.error(f"BrowserManager DOM功能测试出错: {e}")
        import traceback
        traceback.print_exc()
