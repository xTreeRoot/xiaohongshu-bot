...
def example_basic_usage():
    ...
    logger.info(f"插入元素成功: {success}")
    
    # 从数据库获取元素（会自动缓存）
    retrieved_element = dom_manager.get_element("#example-btn")
    if retrieved_element:
        logger.info(f"获取元素成功: {retrieved_element.element_id}")
        logger.info(f"元素描述: {retrieved_element.description}")
    
    # 更新元素
    element.text_content = "更新后的示例按钮"
    update_success = dom_manager.update_element(element)
    logger.info(f"更新元素成功: {update_success}")
...
def example_batch_operations():
    ...
    logger.info("批量插入登录页面元素完成")
    
    # 搜索特定类型的元素
    login_elements = dom_manager.mapper.find_by_type("input")
    logger.info(f"找到 {len(login_elements)} 个input类型的元素")
...
def example_cache_behavior():
    ...
    logger.info("1. 插入元素到数据库")
    
    # 第一次获取（会从数据库加载并存入缓存）
    first_get = dom_manager.get_element("#cache-test")
    logger.info(f"2. 首次获取: {first_get.element_id if first_get else 'None'}")
    
    # 第二次获取（应该从缓存获取）
    second_get = dom_manager.get_element("#cache-test")
    logger.info(f"3. 二次获取: {second_get.element_id if second_get else 'None'}")
    
    # 更新元素后获取（应该获取到更新后的数据）
    element.text_content = "更新后的缓存测试元素"
    dom_manager.update_element(element)
    logger.info("4. 更新元素到数据库")
    
    updated_get = dom_manager.get_element("#cache-test")
    logger.info(f"5. 更新后获取: {updated_get.text_content if updated_get else 'None'}")


if __name__ == "__main__":
    logger.info("DOM管理器使用示例")
    logger.info("=" * 50)
